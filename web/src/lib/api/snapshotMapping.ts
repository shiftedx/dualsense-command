// Pure snapshot mapping: agent DTOs -> UI `AppSnapshot`, plus socket-frame
// classification. No transport, no I/O — everything here is a total function of
// its input, so it can be exercised directly from fixtures (see
// web/scripts/snapshot-mapping.mjs). The transport layer lives in ./snapshot.
import type {
  AdapterStatus,
  AppSettingsResponse,
  AppSnapshot,
...
/**
 * Classification of one raw WebSocket frame, independent of the transport.
 * - `snapshot`: a complete payload, already mapped to an `AppSnapshot`.
 * - `invalidate`: a hint that the client should refetch (incl. a `type:snapshot`
 *   frame whose payload is incomplete).
 * - `ignore`: a benign non-snapshot frame (ping/pong/empty type).
 * - `malformed`: not a string, unparseable JSON, or not an object.
 */
...
/**
 * Classify a raw socket frame without any transport state. The transport calls
 * this and decides what to do with each kind; mapping happens here so the whole
 * decision tree (incl. the previously-silent malformed/invalidate split) is
 * testable from fixtures.
 */
export function classifySnapshotFrame(raw: unknown): SnapshotFrame {
  ...
  const message = value as SnapshotSocketMessage;
  if (message.type === 'snapshot') {
    return isCompleteSnapshotPayload(message.snapshot)
      ? { kind: 'snapshot', snapshot: mapSnapshotDto(message.snapshot) }
      : { kind: 'invalidate' };
  }
  if (isInvalidationMessage(message)) return { kind: 'invalidate' };
  return { kind: 'ignore' };
}
